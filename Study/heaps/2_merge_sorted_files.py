...
# Create a min heap of by getting the smallest(first) value from all the sorted array
# Pop the min element from the min_heap and then check which array it belonged to
# move the pointer for that array to next element and now push that to the min_heap
# repeat step 1 until there are no elements in the heap

# Time - O(nlogk) | Space - O(k), where k is the number of arrays
def merge_sorted_arrays(sorted_arrays):
    min_heap = []
    sorted_array_iters = [iter(x) for x in sorted_array]

    # i would represent which array it belonged to 
    # So when popped out we would now which array pointer has to move forward
    ...
    while min_heap:
        ...
        smallest_array_iter = sorted_array_iters[smallest_entry_i]
